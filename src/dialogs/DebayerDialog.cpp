...
void DebayerDialog::onPatternChanged(int /*index*/)
{
    updatePatternLabel();
}
...
void DebayerDialog::updatePatternLabel()
{
    ...
    const QString detected = detectPatternFromHeader();
    if (detected.isEmpty())
...
QString DebayerDialog::detectPatternFromHeader()
{
    ...
    // Search a priority-ordered list of standard Bayer pattern keywords.
    ...
    {
        if (!keys.contains(hdr.key)) continue;

        QString val = hdr.value.toUpper().trimmed();
        val.remove(',').remove(' ').remove('/').remove('|');

        const QStringList validPatterns = { "RGGB", "BGGR", "GRBG", "GBRG" };

        if (validPatterns.contains(val))
            return val;

        // Also handle values where the pattern is embedded in a longer string.
        for (const QString& pat : validPatterns)
        {
            if (val.contains(pat))
                return pat;
        }
...
QString DebayerDialog::autoDetectByScoring()
{
    ...
    // Debayer each candidate pattern and select the one that minimises the
    // colour imbalance score (lower = more neutral, therefore more likely correct).
    QString bestPattern = "RGGB";
    float   bestScore   = std::numeric_limits<float>::max();

    for (const char* const pat : { "RGGB", "BGGR", "GRBG", "GBRG" })
    {
        const ImageBuffer rgb = ChannelOps::debayer(buf, pat, "bilinear");
        if (!rgb.isValid()) continue;

        const float score = ChannelOps::computeDebayerScore(rgb);
        if (score < bestScore)
        {
            bestScore   = score;
            bestPattern = pat;
...
void DebayerDialog::onApply()
{
    ...
    if (buf.channels() >= 3)
    {
        QMessageBox::information(
            this, tr("Debayer"), tr("Image already has 3 channels."));
        return;
    }
    ...
    if (buf.channels() != 1)
    {
        QMessageBox::warning(
            this, tr("Debayer"),
            tr("Only single-channel mosaic images can be debayered."));
        return;
    }

    // Resolve the Bayer pattern; fall back to scoring if Auto is selected
    // and no header keyword is available.
    ...
    const ImageBuffer rgb = ChannelOps::debayer(
        buf, pattern.toStdString(), method.toStdString());
    ...
    // Commit the result to the viewer with an undo snapshot.
